# Mechanics Clone Spec

Use `data/castle_wars_rules.json` as the canonical rules file.
...
- At start of turn, generate resources equal to circle counts.
- Source behavior: no resource generation during the first two turns.
- Player may cast exactly one card or discard up to three cards.
- Player cannot cast after discarding.
- After card resolution, played card returns to deck and turn advances.
- After discarding fewer than three cards, player may end turn manually.
- Replacement cards are dealt after turn end to restore hand size to 8.

## Damage

Normal damage first reduces Ward. If damage exceeds Ward, leftover reduces Citadel.

Bypass damage ignores Ward and reduces Citadel directly.

Magic Defence negates the next incoming attack before Ward/Citadel resolution.
